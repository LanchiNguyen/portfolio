...
// Historical source remains in Git; only the current compiled app is published.
fs.rmSync(path.join(output, 'v3'), {recursive:true, force:true});
for (const file of [...scripts, 'v4/app/data.js']) fs.rmSync(path.join(output, file), {force:true});
